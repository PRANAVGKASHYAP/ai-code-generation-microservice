  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const formattedMessages: ChatMessage[] = (history as any[]).map((msg) => ({
          id: msg.id.toString(),
          role: msg.messageRole === "USER" ? "user" : "assistant",
          content: msg.content,
          createdAt: msg.createdAt,
          events: msg.events ? msg.events.map((e: any) => ({
            ...e,
            type: e.chatEventType
          })) : [],
        }));
  ...
  const handleSendMessage = useCallback((content: string) => {
    ...
    const cleanup = api.streamChat(
      ...
      (error) => {
        // Handle error
        toast({
          title: "Chat / Connection completed",
          description: error.message === "network error" ? "Stream finished." : error.message,
        });
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === aiMessageId
              ? { ...msg, content: msg.content + "\n\n[Connection closed]", isStreaming: false }
              : msg
          )
        );
        setIsStreaming(false);
    ...
    <div className="h-screen flex flex-col overflow-hidden bg-background">
      ...
      <header className="h-12 shrink-0 border-b border-border/50 bg-panel flex items-center justify-between px-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {project?.role !== 'VIEWER' && (
            <>
              <Button variant="outline" size="sm" className="h-8 text-xs" onClick={() => navigate("/settings")}>
                Upgrade
              </Button>
